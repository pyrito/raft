...
bool moreHealthyChainExists(struct raft *r) {
   ...
   TracefL(INFO, "Chain modification: moreHealthyChainExists cnt_alive_nodes=%d current_chain_len=%d", cnt_alive_nodes, current_chain_len);
   if (cnt_alive_nodes > current_chain_len) {
     return true;
   }
   return false;
}
...
void reformChain(struct raft *r) {
   ...
   */
   ...
   TracefL(INFO, "Chain modification: reformChain cnt_alive_nodes: %d", cnt_alive_nodes);

   raft_index *next_indices;
   chainNode *nodes;
   nodes = (chainNode *) malloc(sizeof(chainNode) * cnt_alive_nodes);
   ...
   qsort(nodes, cnt_alive_nodes, sizeof(chainNode), compChainNode);

   r->chain_incarnation_id++;

   // Send relink messages
   for (int i=0; i<cnt_alive_nodes-1; i++) {
     TracefL(INFO, "Calling relink for nodes[i].id: %d, next_sibling: %d", nodes[i].id,  nodes[(i+1)%cnt_alive_nodes].id);
     if (sendRelink(r, nodes[i].id, nodes[(i+1)%cnt_alive_nodes].id) != 0) {
       return;
     } 
   }
   if (sendRelink(r, nodes[cnt_alive_nodes-1].id, r->id) != 0) {
     return;
   }
   ...
   for (int i = 0; i <cnt_alive_nodes-1; i++) {
      ...
      progress->next_sibling_id = nodes[(i+1)%cnt_alive_nodes].id;
   }

   int node_idx = configurationIndexOf(&r->configuration, nodes[cnt_alive_nodes-1].id);
   struct raft_progress *progress = &(r->leader_state.progress[node_idx]);
   ...
   for (int i = 0; i <cnt_alive_nodes; i++) {
      int node_idx = configurationIndexOf(&r->configuration, nodes[i].id);
      struct raft_progress *progress = &(r->leader_state.progress[node_idx]);
      progress->next_index = nodes[i].next_index;
   }

   char* node_ids_str = malloc(3*cnt_alive_nodes*sizeof(char));
   if (node_ids_str == NULL) {
     TracefL(ERROR, "Failed to malloc");
   }
   memset(node_ids_str, 0, 3*cnt_alive_nodes*sizeof(char));
   for (int i = 0; i <cnt_alive_nodes; i++) {
     char str[12] = {0};
     sprintf(str, "%d, ", nodes[i].id);
     strcat(node_ids_str, str);
   }
   fprintf(stderr, "Chain modification: New chain incarnation id %d with nodes: %s\n", r->chain_incarnation_id, node_ids_str);
